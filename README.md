...
4. **`ValueOf<T>`**: Получает тип значения свойства из объекта `T`:
   ...
   ```
## 7) создание пользовательских типов
...
4. **Ограничение типов**:
   ...
   ```
## 9) понимание TS (ES6) модульной системы
...
Этот полифилл проверяет, есть ли метод `bind` у объекта `Function.prototype`. Если он отсутствует, то он создает функцию `bind`, которая принимает первый аргумент в качестве контекста, а остальные аргументы будут использоваться как аргументы, передаваемые в исходную функцию. Возвращаемая функция будет вызывать исходную функцию с переданным контекстом и аргументами.
...
boundGreet('Alice'); // Выведет: Hello, Alice! My name is John.
```
